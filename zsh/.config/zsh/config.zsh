...
if [[ $TERM == dumb || -n $INSIDE_EMACS ]]; then
    ...
    whence -w preexec >/dev/null && unfuction preexec
    PS1='$ '
fi

## Plugins
# zgen
# we ahndle compinit ourselves...
...
## ZSH configuration
# Treate these characters as part of a word.
WORDCHARS='_-*?[]~&.;!#$%^(){}<>'
...
setopt AUTO_RESUME	# Attempt to resume existing job before create a new process.
setopt NOTIFY		# Repost status of background jobs immediately.
...
setopt HIST_BEEP		# Beep when accssing non-existent history.
